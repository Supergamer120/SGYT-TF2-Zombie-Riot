...
void AlligaOnMapStart()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Alliga");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_alliga");
...
{
	return Alliga(vecPos, vecAng, team);
}

methodmap Alliga < CClotBody
{
	...
	public Alliga(float vecPos[3], float vecAng[3], int ally)
	{
		Alliga npc = view_as<Alliga>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.0", "20000", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(Alliga_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Alliga_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Alliga_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/sniper/hwn2018_crocodile_mun_dee/hwn2018_crocodile_mun_dee.mdl");
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/dec23_rugged_rags/dec23_rugged_rags.mdl");
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/sniper/hwn2018_scopers_scales/hwn2018_scopers_scales.mdl");
		...
		SetEntityRenderColor(npc.m_iWearable2, 56, 66, 72, 255);
		SetEntityRenderColor(npc.m_iWearable3, 56, 66, 72, 255);
		SetEntityRenderColor(npc.m_iWearable4, 56, 66, 72, 255);
...
public void Alliga_ClotThink(int iNPC)
{
	Alliga npc = view_as<Alliga>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTargetWalkTo))
	{
		...
		int ExtraBehavior = AlligaSelfDefense(npc,GetGameTime(npc.index)); 
...
public Action Alliga_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Alliga npc = view_as<Alliga>(victim);
...
public void Alliga_NPCDeath(int entity)
{
	Alliga npc = view_as<Alliga>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int AlligaSelfDefense(Alliga npc, float gameTime)
{
	...
	if(npc.m_flAttackHappens)
	{
		if(npc.m_flAttackHappens < gameTime)
		{
			...
			if(IsValidEnemy(npc.index, target))
			{
				...
				ApplyStatusEffect(npc.index, target, "Alliga's Posion", Rogue_Paradox_RedMoon() ? 15.0 : 5.0);
			} 
		}
	}
