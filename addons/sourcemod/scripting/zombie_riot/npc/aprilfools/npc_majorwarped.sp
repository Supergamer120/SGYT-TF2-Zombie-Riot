...
methodmap MajorWarped < CClotBody
{
	...
	public MajorWarped(float vecPos[3], float vecAng[3], int ally)
	{
		...
		SetEntPropFloat(npc.index, Prop_Data, "m_flElementRes", 0.5, Element_Void);
	...
	//	AcceptEntityInput(npc.index, "SetBodyGroup");
		...
		SetEntityRenderColor(npc.index, 188, 221, 179, 255);
		SetEntityRenderColor(npc.m_iWearable2, 188, 221, 179, 255);
	...
	if(target > 0)
	{
		...
		if(npc.m_flAttackHappens)
		{
			if(npc.m_flAttackHappens < gameTime)
			{
				...
				if(entity != -1)
				{
					i_VoidArrowAmount[entity] = 60;
					i_ChaosArrowAmount[entity] = 60;
