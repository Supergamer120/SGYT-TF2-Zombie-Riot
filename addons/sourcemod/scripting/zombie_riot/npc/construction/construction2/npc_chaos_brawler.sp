...
void ChaosBrawler_OnMapStart_NPC()
{
...
methodmap ChaosBrawler < CClotBody
{
	...
	public ChaosBrawler(float vecPos[3], float vecAng[3], int ally)
	{
		ChaosBrawler npc = view_as<ChaosBrawler>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.15", "1500", ally));
		SetVariantInt(1);
		...
		func_NPCDeath[npc.index] = ChaosBrawler_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ChaosBrawler_OnTakeDamage;
		func_NPCThink[npc.index] = ChaosBrawler_ClotThink;
		...
		npc.m_iWearable1 = ParticleEffectAt_Parent(flPos, "unusual_smoking", npc.index, "eyes", {0.0,0.0,0.0});
		npc.m_iWearable2 = ParticleEffectAt_Parent(flPos, "unusual_psychic_eye_white_glow", npc.index, "eyes", {0.0,0.0,-15.0});
		npc.StartPathing();
...
public void ChaosBrawler_ClotThink(int iNPC)
{
	ChaosBrawler npc = view_as<ChaosBrawler>(iNPC);
...
public Action ChaosBrawler_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	ChaosBrawler npc = view_as<ChaosBrawler>(victim);
...
public void ChaosBrawler_NPCDeath(int entity)
{
	ChaosBrawler npc = view_as<ChaosBrawler>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
